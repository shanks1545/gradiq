...
async function viewTest(testId) {
    try {
        ...
        document.getElementById('print-results-btn').onclick = () => printResults(test, results);
...
const DEFAULT_WA_TEMPLATE = '*{institute_name} Result*\nDear Parent,\n{student_name} scored {score}/{total} ({percentage}%) — Grade {grade} in {test_name}.\n_Sent via GradiQ_';

function getWaTemplate() {
    return localStorage.getItem('gradiq_wa_template') || DEFAULT_WA_TEMPLATE;
}

function formatWaTemplate(vars) {
    let text = getWaTemplate();
...
document.getElementById('weekly-report-btn').addEventListener('click', async () => {
    try {
        weeklyData = await api('/api/weekly-report');
        const inst = instituteName || 'Your Institute';
        document.getElementById('weekly-title').textContent = `This Week at ${inst}`;
        const g = document.getElementById('weekly-grid');
        const tp = weeklyData.top_performer;
        g.innerHTML = `
            <div class="result-stat"><span class="stat-label">Students Graded</span><span class="stat-value">${weeklyData.total_students}</span></div>
            <div class="result-stat"><span class="stat-label">Class Average</span><span class="stat-value">${weeklyData.avg_percentage}%</span></div>
            <div class="result-stat"><span class="stat-label">Top Performer</span><span class="stat-value" style="font-size:15px">${tp ? esc(tp.name) + ' — ' + tp.score + '/' + tp.total : 'N/A'}</span></div>
            <div class="result-stat"><span class="stat-label">At-Risk Students</span><span class="stat-value" style="color:${weeklyData.at_risk_count > 0 ? 'var(--red)' : 'var(--green)'}">${weeklyData.at_risk_count}</span></div>`;
        document.getElementById('weekly-modal').classList.remove('hidden');
    } catch (err) { toast(err.message, true); }
});

document.getElementById('close-weekly-btn').addEventListener('click', () => document.getElementById('weekly-modal').classList.add('hidden'));
document.getElementById('close-weekly-btn2').addEventListener('click', () => document.getElementById('weekly-modal').classList.add('hidden'));
document.getElementById('weekly-modal').addEventListener('click', (e) => { if (e.target === e.currentTarget) e.currentTarget.classList.add('hidden'); });

document.getElementById('weekly-wa-btn').addEventListener('click', () => {
    if (!weeklyData) return;
    const inst = instituteName || 'GradiQ';
    const tp = weeklyData.top_performer;
    const text = `*GradiQ Weekly Report — ${inst}*\n\n` +
        `📊 This week's summary:\n` +
        `• Students graded: ${weeklyData.total_students}\n` +
        `• Class average: ${weeklyData.avg_percentage}%\n` +
        `• Top performer: ${tp ? tp.name + ' — ' + tp.score + '/' + tp.total : 'N/A'}\n` +
        `• At-risk students: ${weeklyData.at_risk_count}\n\n` +
        `_Powered by GradiQ_`;
    window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank');
});
...
function printResults(test, results) {
    if (!results || results.length === 0) { toast('No results to print.', true); return; }
    const ranked = [...results].sort((a, b) => b.percentage - a.percentage);
    const inst = instituteName || '';
    const initials = inst ? inst.split(' ').map(w => w[0]).join('').substring(0, 2).toUpperCase() : 'GQ';
    const date = new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' });

    const rows = ranked.map((r, i) =>
        `<tr><td>${i + 1}</td><td>${esc(r.student_name)}</td><td>${r.score} / ${r.total_marks}</td><td>${r.percentage}%</td><td>${esc(r.grade)}</td></tr>`
    ).join('');

    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${esc(test.name)} — Results</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",Arial,sans-serif;color:#111;padding:40px;font-size:12pt;line-height:1.5}
.header{display:flex;align-items:center;gap:16px;margin-bottom:24px;padding-bottom:16px;border-bottom:2px solid #111}
.logo-circle{width:48px;height:48px;border-radius:50%;background:#111;color:#fff;display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:700;flex-shrink:0}
.header-text h1{font-size:18pt;font-weight:700;letter-spacing:-0.02em}
.header-text p{font-size:10pt;color:#666}
.meta{display:flex;gap:24px;font-size:10pt;color:#666;margin-bottom:20px}
table{width:100%;border-collapse:collapse;font-size:10pt}
th{text-align:left;padding:8px 10px;font-weight:700;border-bottom:2px solid #111;font-size:9pt;text-transform:uppercase;letter-spacing:0.05em}
...
<div class="header"><div class="logo-circle">${initials}</div><div class="header-text"><h1>${esc(inst || 'GradiQ')}</h1><p>${esc(test.name)} — ${esc(test.subject)}</p></div></div>
<div class="meta"><span>Date: ${date}</span><span>Subject: ${esc(test.subject)}</span><span>Total Marks: ${test.total_marks}</span><span>Students: ${ranked.length}</span></div>
<table><thead><tr><th>#</th><th>Student Name</th><th>Score</th><th>Percentage</th><th>Grade</th></tr></thead><tbody>${rows}</tbody></table>
<div class="footer">Generated by GradiQ &mdash; gradiq.co.in</div>
...
function updateWaPreview() {
    const template = document.getElementById('settings-wa-template').value;
    const preview = formatWaTemplate({
        student_name: 'Rahul Sharma', score: '18', total: '20',
        percentage: '90', grade: 'A+', test_name: 'Midterm Exam',
...
document.getElementById('settings-wa-template').addEventListener('input', updateWaPreview);
...
document.getElementById('save-settings-btn').addEventListener('click', async () => {
    try {
        ...
        localStorage.setItem('gradiq_institute', instituteName);
        const tmpl = document.getElementById('settings-wa-template').value.trim();
        if (tmpl) localStorage.setItem('gradiq_wa_template', tmpl);
        else localStorage.removeItem('gradiq_wa_template');
        renderTeacherName();
        document.getElementById('settings-modal').classList.add('hidden');
